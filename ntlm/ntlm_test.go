...
func TestNtlmClientSession(t *testing.T) {
	...
	creds := Creds{ username: "DOMAIN\\user", password: "foo"}
    _, err := ntlmClientSession(creds)
	assert.Equal(t, err, nil)
    assert.Equal(t, ntlmSession != nil, true)

	//The second call should ignore creds and give the session we just created.
	badCreds := Creds{username: "badusername", password: "foo"}
	_, err = ntlmClientSession(badCreds)
	assert.Equal(t, err, nil)
...
func TestNtlmClientSessionBadCreds(t *testing.T) {
	...
	creds := Creds{username: "badusername", password: "foo"}
	_, err := ntlmClientSession(creds)
